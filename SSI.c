...
extern void Configurar_SSI2(void)
{
    ...
    //GPIOD->DIR |= (0<<3) | (1<<2) | (1<<1) | (0<<0); //selector es salida = 1 *en la tabla dice que se ignora
    GPIOB->AFSEL = (1<<7)|(1<<6)|(1<<5)|(1<<4); // porque se usa el pin B4-B7
    GPIOB->PCTL = (GPIOD->PCTL&0x0000FFFF) | 0xFFFF0000; // tabla p.689 *se apaga del pin 0-3 y se enciende del pin 4-7
   ...
   // GPIOD_AHB->DATA |= (1<<5); //registrar CS
    ...
    SSI2->CR1 |= (1<<1); //SSE=1 habilitar modoulo p.971 (0x02)
}
